...
Preflight is not a complete interpreter for arbitrary custom macros or raw G-code. Review the exact artifact and perform an elevated, fluid-free dry run after changing workflows, profiles, fixtures, or firmware. The supplied Moonraker configuration permits unauthenticated clients from IPv4 private and link-local networks so any local PC can use Mainsail or OpenSpotter; keep port 7125 off the public Internet.
